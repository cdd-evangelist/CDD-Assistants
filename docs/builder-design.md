...
ラウンドトリップ検証の具体的な実行手順。Opus がオーケストレーターとして以下を実行する。

#### Step 1: サブエージェントによる実装 + リファレンス生成

Opus は Sonnet サブエージェントにチャンクの実装を指示する。サブエージェントは **実装・テスト・リファレンス作成を一括で行う**（§7.1 シーケンス図参照）。

Opus が渡す情報:
```
- チャンクの implementation_prompt（プレースホルダ解決済み）
- expected_outputs（生成すべきファイル一覧）
- completion_criteria（完了条件）
- reference_doc の出力先パス
- 指示: 実装完了後、コードから読み取れる事実のみを日本語でリファレンスに記述せよ
```

サブエージェントが生成するリファレンスに含める項目:
...
```
致命的が1件以上 → サブエージェントに差し戻し（乖離箇所を指示）
                   → リトライ回数チェック → 超過なら人に判断を仰ぐ
致命的が0件     → complete_chunk → 後続チャンクをアンロック
                   → 「要更新」があれば設計文書の更新を提案
```

この一連の手順は MCP ツールではなく、Opus のプロンプトとオーケストレーションで実現する。
