...
#### Step 1: サブエージェントへのリファレンス生成指示

Opus は Sonnet サブエージェントに以下を渡す:

```
- 対象ソースファイル一覧（チャンクの expected_outputs に対応する実装コード）
- 出力先パス（チャンクの reference_doc）
- 指示: 「コードから読み取れる事実のみを日本語で記述せよ」
```
...
**重要: 設計意図の推測はしない。「こうだろう」ではなく「こうなっている」で書く。**
