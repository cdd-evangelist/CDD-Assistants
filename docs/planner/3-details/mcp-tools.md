...
**処理ステップ:**
1. `project_dir` 配下を再帰的にスキャンして .md ファイルを列挙する（後述「スキャン範囲」を参照）
2. 各文書のフロントマター or 先頭セクションからメタデータを抽出する。`README.md` / `CHANGELOG.md` で `layer` 未指定なら `context` 層として扱う
3. 決定事項（Decision）と未決事項（Open Question）を収集する
4. 文書間の参照関係を構築する
5. 進捗サマリーを生成する
6. 設計文書標準のパスを解決する（プロジェクト内 `docs/design-doc-standard.md` → バンドル版フォールバックの順）
7. プロジェクト名を解決する（後述「プロジェクト名の解決」を参照）

**スキャン範囲:**

`project_dir` 配下を再帰的に走査して `.md` ファイルを列挙する。設計文書標準 §5.1 のフォルダ構成（`1-usecases/`, `2-features/`, `3-details/`, `4-ref/`）と §5.4 の複数コンポーネント構成を、利用者がフォルダ深さを意識せずに扱えるようにするため。

ただし以下のディレクトリは降りない:

- `.git`, `node_modules`, `dist`, `build`, `target`
- 先頭が `.` のディレクトリ（隠しディレクトリ全般）

文書のパスは `project_dir` からの相対パスで保存する（例: `2-features/auth.md`、`planner/3-details/mcp-tools.md`）。

**プロジェクト名の解決:**

- `project_dir` の末尾セグメントを使う
- ただし末尾が `docs` の場合は、その親ディレクトリ名を使う（利用者が `docs/` を直接渡したケースで「プロジェクト名が docs」になるのを防ぐ）

例: `/path/to/cdd-term/docs` → `cdd-term`、`/path/to/cdd-term` → `cdd-term`
...
**処理ステップ:**
1. プロジェクトの決定ログ（`decisions.jsonl`）の保存先を解決する（後述「保存先の解決」を参照）
2. 親ディレクトリが存在しなければ自動作成する（`mkdir -p` 相当）
3. 決定ログに追記する
4. 影響する文書にフラグを立てる（要更新）

**保存先の解決:**

`decisions.jsonl` は設計文書標準 §5.1 の `4-ref/`（リファレンス・検証結果・補足資料）配下に置く。`project_dir` の構造に応じて解決する:

| `project_dir` の構造 | 保存先 |
|---|---|
| 配下に `docs/` が存在する | `<project_dir>/docs/4-ref/decisions.jsonl` |
| 末尾が `docs`（`docs/` を直接渡された） | `<project_dir>/4-ref/decisions.jsonl` |
| `docs/` なし（フラット構成） | `<project_dir>/4-ref/decisions.jsonl` |

`design_context` 等の読み込み側も同じロジックで解決する。後方互換は持たない（CDD-Term が初の適用例で、旧仕様 `<project_dir>/decisions.jsonl` を読む必要がない）。
...
設計文書標準 §5.1 の `4-ref/`（リファレンス）配下に自動生成される（保存先の解決は §3.4「保存先の解決」を参照）。壁打ちの記録が蓄積される。
