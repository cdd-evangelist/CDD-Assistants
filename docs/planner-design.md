...
2. **規模判定（コンシェルジュ）**: 入力テキストからワンショットか CDD フルコースかを判定する
3. テンプレート質問（必ず確認する軸）から、キーワードマッチで未回答の項目を選出
4. 未回答の軸に対応するテンプレート質問を選択（各軸から先頭1問、最大4問）
5. 既知の類似プロダクト・アプローチがあればキーワードマッチで参考として提示（最大3件）

**規模判定（コンシェルジュ）:**

入力テキストのキーワードから規模感を推定し、ルートを提案する:
...
- ワンショットのシグナルが強い → `route: "one-shot"` を返し、「壁打ち不要、そのまま実装に進めそうですがどうします？」と提案
- CDD のシグナルが強い、またはどちらとも言えない → `route: "full"` を返し、通常の Phase 0 フローへ
