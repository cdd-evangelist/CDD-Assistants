...
### 1.3.1 対応外のケースと別アシスタント

Builder のスコープは **「設計文書が3階層モデルに沿って整理済み + グリーンフィールド実装」** に限定する。以下のケースは Builder 単体では扱わず、別アシスタントの責務として明示的に分離する（いずれも未実装）。

| ケース | 対応する別アシスタント |
|---|---|
| 設計文書がない / 既存コードから設計を起こしたい | **Archaeologist**（未実装） |
| 設計文書がフラット構造で3階層モデルに整っていない | **Cleaner**（未実装） |
| 既に完了済みのチャンクへ部分追加実装したい | **Refactor**（未実装） |

特にラウンドトリップ検証で「設計書の特定セクションのみ実装漏れ」が判明した場合、これは Refactor の責務領域。完了済 chunk に対して `run_test_agent` / `run_impl_agent` を再 run すると、既存テスト（同 chunk 配下に蓄積されたもの）を上書きするリスクがあるため、Builder の標準フローでは安全に扱えない。

#### 暫定回避策: mini recipe パターン

Refactor が実装されるまでの暫定運用として、未実装サブセクションだけを対象にした **mini recipe** を手作業で組み立てる方法がある。

1. `recipe.json` から該当 chunk の `source_content` をコピー
2. 新規 `recipe-{name}.json` を作成
   - `chunks` は 1 件のみ
   - `implementation_prompt` で「**該当サブセクションのみ実装、既存ファイル・他セクションは触らない**」と明示
3. `load_recipe` → `run_test_agent` → `run_impl_agent` → `complete_chunk` を通常どおり実行

トレードオフ:

- `source_content` を手コピーするため、元設計の整合性ズレに注意
- 並列レシピ運用となるため、`.gitignore` に `recipe-*-state.json` を含めること（state ファイル名が派生命名で生成される）
- 「部分追加」を体系的に扱う仕組みではないので、頻発するなら Refactor の本実装を優先する判断材料にする
