...
8. **コード規約の検出** — `project_dir` で以下を優先度順に探索し、見つかったものを `coding_standards` に記録（[コード規約 §3](../2-features/coding-standards.md) 参照）:
...
Dual-Agent TDD で共有バイアスを防いでも、テストの assert が弱い問題は残る（[テスト品質](../2-features/test-quality.md) 参照）。Mutation Testing は「カバレッジ ≠ テスト有効性」への唯一の自動化対策。Survived（検出されなかった）ミュータントは Test Agent に具体的な強化指示として渡す。
