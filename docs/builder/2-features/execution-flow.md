...
### 4.1 Step 1: Test Agent によるテスト生成（Red フェーズ）
...
### 4.2 Step 2: Impl Agent による実装 + リファレンス生成（Green フェーズ）
...
### 4.3 Step 3: ラウンドトリップ照合

オーケストレーターは以下の 2 文書を読み比べる:
...
照合の仕組みの詳細は [ラウンドトリップ検証 §3](roundtrip-verification.md) を参照。
...
### 4.5 Step 5: Investigation Agent による原因仕分け（照合 NG 時のみ）
...
テストと実装のコンテキストを分離することで共有バイアスを排除し、リファレンスを設計文書から独立させることでラウンドトリップ検証の自己参照を防ぐ。分離の意義と実例は §5 設計判断を参照。
